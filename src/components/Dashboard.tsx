  ...
  return (
    <div className="flex flex-col md:flex-row h-full w-full">
      ...
      <div className="flex-1 pr-0 md:pr-10 pb-10 md:pb-0 flex flex-col font-sans">
        <div className="mb-12">
          <h3 className="text-[60px] md:text-[80px] font-black leading-[0.85] tracking-tighter mb-4 italic text-[#FAFAFA]">
            PROCESS<br/><span className="text-transparent border-b-4 border-cyan-500" style={{WebkitTextStroke: "1px white"}}>PAYROLL.</span>
          </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              <label className="text-[11px] uppercase tracking-widest text-[#737373] font-bold">1. Employee Pubkey</label>
              <input 
                value={employeeWallet}
                onChange={e => setEmployeeWallet(e.target.value)}
                placeholder="5NKPWTom5..."
                className="w-full bg-transparent border-b-2 border-[#262626] text-white font-mono outline-none focus:border-cyan-500 pb-2"
              />
              <button
                onClick={handleOnboard}
                disabled={loading || !wallet || !employeeWallet}
                className="w-full border border-dashed border-[#262626] py-3 hover:bg-[#171717] text-[10px] uppercase font-bold text-[#525252] transition-colors disabled:opacity-50"
              >
                + Onboard Employee
              </button>
            </div>
            ...
            {/* Salary form */}
            <div className="space-y-4">
               <label className="text-[11px] uppercase tracking-widest text-[#737373] font-bold">2. Plaintext Input (USDC)</label>
                <input 
                  type="text" 
                  value={amount}
                  onChange={e => setAmount(e.target.value)}
                  placeholder="5000.00" 
                  className="w-full bg-transparent border-b-2 border-white text-[#FAFAFA] text-4xl font-black outline-none focus:border-cyan-500 pb-2"
                />
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-[10px] text-[#404040] font-mono">ENCRYPTION_STATUS: {loading ? 'PROCESSING' : 'IDLE'}</span>
                <div className="h-[1px] flex-1 bg-[#262626]"></div>
              </div>
              <button 
                onClick={handleProcessPayroll}
                disabled={loading || !wallet || !amount || !employeeWallet}
                className="w-full h-16 bg-white text-black font-black text-lg uppercase tracking-tighter hover:bg-cyan-500 transition-colors transform hover:-translate-y-1 disabled:opacity-50 disabled:hover:translate-y-0"
              >
          ...
          <div className="bg-[#0A0A0A] border border-[#262626] p-6 relative overflow-hidden flex flex-col justify-between">
            <div className="absolute top-0 right-0 p-2 text-[8px] font-mono text-cyan-500/50">SECURE_ENCLAVE_V1</div>
            <div className="text-[10px] uppercase font-bold text-[#737373] mb-4">Ciphertext Buffer [96b]</div>
            <div className="font-mono text-[10px] text-cyan-500/80 break-all leading-relaxed">
              {ciphertextDisplay}
            </div>
          </div>
        </div>
      ...
      <aside className="w-full md:w-64 border-t md:border-t-0 md:border-l border-[#262626] bg-[#0A0A0A] p-6 flex flex-col font-mono text-[10px]">
        <div className="text-[10px] uppercase tracking-[0.2em] text-[#737373] font-bold mb-6 font-sans">Network Log</div>
        
        <div className="flex-1 space-y-4 overflow-y-auto">
          {logs.map((log, i) => (
             <div key={i} className={log.includes('[SUCCESS]') || log.includes('[ARCIUM]') || log.includes('[SYSTEM]') ? 'text-cyan-500' : log.includes('ERROR') ? 'text-red-500' : 'text-[#525252]'}>
               {log}
             </div>
          ))}

          <div className="p-3 bg-[#111] border border-[#262626] space-y-2 mt-4">
            <div className="text-[#737373] uppercase">MXE_PUBLIC_KEY</div>
            <div className="break-all text-[#A3A3A3]">Arcj82pX7HxYKLR92qvgZUAd7vGS1k4hQvAFcPATFdEQ</div>
          </div>

          <div className="mt-10 border-t border-[#262626] pt-6">
            <div className="text-[#737373] uppercase mb-2">PDA SEEDS REFERENCE</div>
        ...
        <div className="pt-4 mt-4 border-t border-[#262626] shrink-0">
          <div className="flex items-center justify-between text-[9px] font-bold uppercase">
            <span className="text-[#404040]">Version</span>
            <span className="text-[#FAFAFA]">Arcium v0.9.7</span>
          </div>
        </div>
      </aside>
    </div>
